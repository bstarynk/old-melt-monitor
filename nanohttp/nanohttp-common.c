...
herror_new (const char *func, int errcode, const char *format, ...)
{
  ...
  herror_impl_t *impl = (herror_impl_t *) GC_MALLOC (sizeof (herror_impl_t));
...
hpairnode_new (const char *key, const char *value, hpair_t * next)
{
  ...
  pair = (hpair_t *) GC_MALLOC (sizeof (hpair_t));

  if (key != NULL)
    {
      pair->key = (char *) GC_MALLOC (strlen (key) + 1);
      strcpy (pair->key, key);
    }
  else
    {
  ...
  if (value != NULL)
    {
      pair->value = (char *) GC_MALLOC (strlen (value) + 1);
...
hpairnode_parse (const char *str, const char *delim, hpair_t * next)
{
  ...
  if (key != NULL)
    {
      pair->key = (char *) GC_MALLOC_ATOMIC (strlen (key) + 1);
      strcpy (pair->key, key);
    }
  if (value != NULL)
    {
      for (c = 0; value[c] == ' '; c++);	/* skip white space */
      pair->value = (char *) GC_MALLOC_ATOMIC (strlen (&value[c]) + 1);
...
content_type_new (const char *content_type_str)
{
  ...
  /* Create object */
  ct = (content_type_t *) GC_MALLOC (sizeof (content_type_t));
  ct->params = NULL;
...
part_new (const char *id, const char *filename,
	  ...
	  part_t * next)
{
  part_t *part = (part_t *) GC_MALLOC (sizeof (part_t));
  part->header = NULL;
  part->next = next;
  part->deleteOnExit = 0;
  strcpy (part->id, id);
...
attachments_new ()		/* should be used internally */
{
  attachments_t *attachments =
    (attachments_t *) GC_MALLOC (sizeof (attachments_t));
