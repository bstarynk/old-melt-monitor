...
http_input_stream_new (hsocket_t * sock, hpair_t * header)
{
  ...
  if (!
      (result =
       (http_input_stream_t *) GC_MALLOC (sizeof (http_input_stream_t))))
    {
      log_error2 ("Gcmalloc failed (%s)", strerror (errno));
      return NULL;
    }
...
_http_input_stream_chunked_read_chunk_size (http_input_stream_t * stream)
{
  ...
  while (1)
      ...
      if (i == 24)
	{
	  stream->err =
	    herror_new ("_http_input_stream_chunked_read_chunk_size",
			STREAM_ERROR_NO_CHUNK_SIZE, "reached max line == %d",
			i);
	  return -1;
	}
      else
  ...
  while (size > 0)
      ...
      if (remain == 0 && stream->chunk_size != -1)
	{
	  ...
	  while (1)
	    {
	      if ((err =
		   hsocket_read (stream->sock, &ch, 1, 1, &status)) != H_OK)
		{
		  stream->err = err;
		  return -1;
		}
      ...
      if (remain < size)
	{
	  /* read from socket */
	  if ((err =
	       hsocket_read (stream->sock, &(dest[read]), remain, 1,
			     &status)) != H_OK)
	    {
	      stream->err = err;
	      return -1;
...
*/
  ...
  if (!
      (result =
       (http_output_stream_t *) GC_MALLOC (sizeof (http_output_stream_t))))
    {
      log_error2 ("Gcmalloc failed (%s)", strerror (errno));
      return NULL;
    }
