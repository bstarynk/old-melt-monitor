...
httpc_new (void)
{
  ...
  if (!(res = (httpc_conn_t *) GC_MALLOC (sizeof (httpc_conn_t))))
    return NULL;
...
static int
_httpc_set_basic_authorization_header (httpc_conn_t * conn, const char *key,
...
int
httpc_set_basic_authorization (httpc_conn_t * conn, const char *user,
			       const char *password)
{
  return _httpc_set_basic_authorization_header (conn, HEADER_AUTHORIZATION,
						user, password);
}

int
httpc_set_basic_proxy_authorization (httpc_conn_t * conn, const char *user,
				     const char *password)
{
...
httpc_get (httpc_conn_t * conn, hresponse_t ** out, const char *urlstr)
{
  ...
  if ((status =
       httpc_talk_to_server (HTTP_REQUEST_GET, conn, urlstr)) != H_OK)
    return status;
