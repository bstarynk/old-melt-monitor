...
static in_addr_t _http_host = INADDR_ANY;

/* added by Basile */
void
hsocket_basile_set_host (const char *hoststr)
{
  if (!hoststr)
    return;
  if (!strcmp (hoststr, "localhost"))	// this is the most common case
    {
      _http_host = INADDR_LOOPBACK;
      return;
    }
  ...
  if (getaddrinfo (hoststr, NULL, &hints, &res))
    log_error3 ("cannot get address of %s : %s", hoststr, strerror (errno));
  int sz = sizeof (_http_host);
  // this probably doe not have a lot of sense, but might sometimes
  // make sense at least for IPv4, like gethostbyname did in the
  // past...
  if (res)
    {
      if (sz > res->ai_addrlen)
	sz = res->ai_addrlen;
      memcpy (&_http_host, res->ai_addr, sz);
    }
}
...
hsocket_bind (hsocket_t * dsock, int port)
{
  ...
  addr.sin_addr.s_addr = _http_host;
  ...
  if (listen (sock->sock, 15) == -1)
    {
      ...
      return herror_new ("hsocket_listen", HSOCKET_ERROR_LISTEN,
			 "Cannot listen on this socket (%s)",
			 strerror (errno));
    }
