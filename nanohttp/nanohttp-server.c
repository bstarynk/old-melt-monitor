...
void
httpd_basile_set_port (const char *portstr)
{
  if (portstr)
    _httpd_port = atoi (portstr);
}
...
_httpd_connection_slots_init (void)
{
  ...
  _httpd_connection_lock =
    CreateMutex (NULL, TRUE, _httpd_connection_lock_str);
#else
  pthread_mutex_init (&_httpd_connection_lock, NULL);
#endif
  _httpd_connection =
    GC_MALLOC (_httpd_max_connections * sizeof (conndata_t));
  memset (_httpd_connection, 0, _httpd_max_connections * sizeof (conndata_t));
...
httpd_register_secure (const char *ctx, httpd_service func, httpd_auth auth)
{
  ...
  if (!(service = (hservice_t *) GC_MALLOC (sizeof (hservice_t))))
    {
...
{
  memset (service, 0, sizeof (hservice_t));
  GC_FREE (service);
...
httpd_send_header (httpd_conn_t * res, int code, const char *text)
{
  ...
  /* set server name */
  // Basile changes it
  strcat (header, "Server: Nano HTTPD library For MELT monitor\r\n");
  ...
  /* send header */
  if ((status =
       hsocket_nsend (res->sock, (const byte_t *) header,
		      strlen (header))) != H_OK)
    return status;
...
httpd_send_internal_error (httpd_conn_t * conn, const char *errmsg)
{
  ...
  snprintf (buflen, sizeof (buflen), "%d", (int) strlen (buffer));
...
httpd_new (hsocket_t * sock)
{
  ...
  if (!(conn = (httpd_conn_t *) GC_MALLOC (sizeof (httpd_conn_t))))
    {
...
_httpd_decode_authorization (const char *value, char **user, char **pass)
{
  ...
  len = strlen (value) * 2;
  if (!(tmp = (unsigned char *) GC_MALLOC_ATOMIC (len)))
    {

      log_error2 ("Gc_Malloc_Atomic failed (%s)", strerror (errno));
      return -1;
    }
  ...
  base64_decode ((unsigned char *) value, tmp);
  ...
  if ((tmp2 = (unsigned char *) strstr ((char *) tmp, ":")))
    {
      *tmp2++ = '\0';
      *pass = GC_STRDUP ((char *) tmp2);
    }
  else
    {
      *pass = GC_STRDUP ("");
    }
  *user = (char *) GC_STRDUP ((char *) tmp);
...
#endif
{
  ...
  while (!done)
    {
      log_verbose3 ("starting HTTP request on socket %p (%d)", conn->sock,
		    conn->sock.sock);
      ...
      else
	{
	  ...
	  if ((service = httpd_find_service (req->path)))
	    {
	      log_verbose3 ("service '%s' for '%s' found", service->ctx,
			    req->path);
	      ...
	      else
		{
		  char *template =
		    ...
		    "</head>"
		    "<body>"
		    "<h1>Unauthorized request logged</h1>" "</body>"
		    "</html>";
...
_httpd_start_thread (conndata_t * conn)
{
  ...
  // Basile use GC_pthread_sigmask & GC_pthread_create here
  GC_pthread_sigmask (SIG_BLOCK, &thrsigset, NULL);
  if ((err =
       GC_pthread_create (&(conn->tid), &(conn->attr), httpd_session_main,
			  conn)))
  ...
  for (;;)
      ...
      for (i = 0; i < _httpd_max_connections; i++)
	{
	  if (_httpd_connection[i].tid == 0)
	    continue;
	  GetExitCodeThread ((HANDLE) _httpd_connection[i].tid,
			     (PDWORD) & rc);
	  if (rc != STILL_ACTIVE)
	    continue;
	  connections++;
		    ...
		    long max)
{
  ...
  if (content_length == 0)
    {
      *received = 0;
      if (!(postdata = (unsigned char *) GC_MALLOC (1)))
	{
  ...
  if (!(postdata = (unsigned char *) GC_MALLOC (content_length + 1)))
    {
      log_error2 ("Gcmalloc failed (%)", strerror (errno));
      return NULL;
    }
		      ...
		      const char *filename)
{
  ...
  while (!feof (fd))
      ...
      if (size == -1)
	{
      ...
      if ((status =
	   http_output_stream_write (conn->out, buffer, size)) != H_OK)
	{
	  fclose (fd);
	  return status;
	}
