	  ...
	  if (ch[0] == '\r')
	      ...
	      if (ch[0] == '\n')
		{
		  if (!MIME_buffer_is_empty (&buffer))
		    {
		      /* Invoke callback */
		      callbacks->received_bytes_cb (callbacks_userdata,
...
mime_streamreader_function (void *userdata, unsigned char *dest, int *size)
{
  ...
  if (readed == -1)
    {
      log_error4 ("[%d] %s():%s ", herror_code (in->err),
		  herror_func (in->err), herror_message (in->err));
    }
...
_mime_part_begin (void *data)
{
  ...
  part = (part_t *) GC_MALLOC (sizeof (part_t));
  part->next = NULL;
		    ...
		    const char *boundary, const char *dest_dir)
{
  ...
  mime_callback_data_t *cbdata = (mime_callback_data_t *)
    GC_MALLOC (sizeof (mime_callback_data_t));
  ...
  message = (attachments_t *) GC_MALLOC (sizeof (attachments_t));
  ...
  if (status == MIME_PARSER_OK)
  ...
  else
    {
      log_error2 ("MIME parser error '%s'!",
		  status ==
		  MIME_PARSER_READ_ERROR ? "read error" :
		  "Incomplete message");
      return NULL;
    }
}
