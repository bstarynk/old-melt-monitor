...
step_tasklet_mom (momitem_t *tkitm, struct mom_taskletdata_st *itd)
{
  ...
  mom_tfunrout_sig_t *routcod = NULL;
  if (!fratop)
    {
      MOM_DEBUG (run,
  ...
  if (curclotyp == momty_item)
      ...
      if (!closd)
	{
      ...
      MOM_DEBUG (run,
		 ...
		 MOMOUT_LITERALV (rdescr->tfun_ident), NULL);
    }
      ...
      if (routitm->i_paylkind != mompayk_routine)
	{
      ...
      if (MOM_UNLIKELY
	  (!rdescr || rdescr->tfun_magic != MOM_TFUN_MAGIC
	   || !rdescr->tfun_codefun))
	{
	  MOM_DEBUG (run,
      ...
      MOM_DEBUG (run,
		 ...
		 MOMOUT_LITERALV (rdescr->tfun_ident), NULL);
      if (curclov.pnode->slen < rdescr->tfun_minclosize)
	{
	  popframe = true;
	  mom_unlock_item (routitm);
	  goto end;
...
jsonrpc_processor_mom (void *p)
{
  ...
  MOM_DEBUGPRINTF (run,
		   "ending jsonrpc_processor_mom socket#%d", jp->jrpc_socket);
  {
    pthread_mutex_lock (&jrpcmtx_mom);
