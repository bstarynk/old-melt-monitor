...
append_collected_value_mom (struct momitemvec_st *ivec, const momvalue_t val)
{
  if (!ivec)
    ivec = mom_itemvec_reserve (NULL, 10);
  switch (val.typnum)
    {
    case momty_item:
      ivec = mom_itemvec_append1 (ivec, val.vitem);
      break;
    case momty_set:
    case momty_tuple:
...
mom_collect_meta_tuple (momvalue_t metav, unsigned nbvals, ...)
{
  ...
  struct momitemvec_st *ivec = mom_itemvec_reserve (NULL, siz);
...
mom_setv_contains (const momvalue_t vset, const momitem_t *itm)
{
  ...
  while (lo + 8 < hi)
    {
      md = (lo + hi) / 2;
      const momitem_t *elemitm = seq->arritm[md];
      assert (elemitm != NULL);
