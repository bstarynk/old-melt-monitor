...
momhash_t mom_string_hash(const char*str, int len)
{
  ...
  int l=len;
  while (l>4) {
    h1 = (509*h2 + 307*((signed char*)str)[0]) ^ (1319*((signed char*)str)[1]);
    h2 = (17*l+5+5309*h2) ^ ((3313*((signed char*)str)[2]) + 9337*((signed char*)str)[3] + 517);
    l -= 4;
    str += 4;
  }
  if (l>0) {
	...
	if (l>3) {
	  h2 = (h2*7727+127)^(313+547*((signed char*)str)[3]);
	}
      }
    }
  }
