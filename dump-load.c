...
#define MOM_DUMP_VERSION "MoniMelt2014A"
...
add_dumped_item (struct mom_dumper_st *dmp, const mom_anyitem_t * itm)
{
  ...
  // unreached
  MOM_FATAL ("corrupted dump of %d items", dmp->dmp_count);
}
...
mom_dump_scan_value (struct mom_dumper_st *dmp, const momval_t val)
{
  ...
  if (MOM_UNLIKELY (dmp->dmp_magic != DUMPER_MAGIC))
    MOM_FATAL ("bad dumper@%p when dumping value @%p", (void *) dmp, val.ptr);
  if (MOM_UNLIKELY (dmp->dmp_state != dus_scan))
    MOM_FATAL ("invalid dump state #%d", (int) dmp->dmp_state);
			...
			const struct momseqitem_st *si)
{
  ...
  if (slen <= TINY_MAX)
    {
      momval_t tab[TINY_MAX] = { MOM_NULLV };
...
#define STRING_CHUNK_SIZE 40
    ...
    default:
      if (typ > momty__itemlowtype && val.panyitem->i_space > 0
	  && val.panyitem->i_space < MOM_SPACE_MAX
	  && mom_spacedescr_array[val.panyitem->i_space])
	{
	  ...
	  if (MOM_UNLIKELY (!found_dumped_item (dmp, val.panyitem)))
	    MOM_FATAL ("unknown dumped item @%p uuid %s", val.panyitem, ustr);
	  if (MOM_UNLIKELY (spadecr->spa_magic != SPACE_MAGIC))
	    MOM_FATAL ("dumped item @%p uuid %s has bad space #%d",
		       val.ptr, ustr, spacenum);
	  if (MOM_UNLIKELY (mom_spacename_array[spacenum] == NULL))
...
mom_load_item (struct mom_loader_st *ld, uuid_t uuid, const char *space)
{
  ...
  if (itm)
    return itm;
  for (spanum = 0; spanum < MOM_SPACE_MAX; spanum++)
    {
      struct momspacedescr_st *curspa = mom_spacedescr_array[spanum];
      if (!curspa)
	continue;
      assert (spanum > 0);
      if (MOM_UNLIKELY (curspa->spa_magic != SPACE_MAGIC))
	MOM_FATAL ("corrupted space #%d", (int) spanum);
	  ...
	  if (buildstr && buildstr[0])
	    {
	      ...
	      if (MOM_UNLIKELY (!fm))
		MOM_FATAL ("fmemopen failed for build string %s", buildstr);
	      ...
	      if (MOM_UNLIKELY (!jval.ptr && errmsg))
		MOM_FATAL
...
mom_load_any_item_data (struct mom_loader_st *ld, mom_anyitem_t * itm,
			momval_t jsob)
{
  if (MOM_UNLIKELY (!ld || ld->ldr_magic != LOADER_MAGIC))
    MOM_FATAL ("invalid loader @%p to fill item data", ld);
  if (MOM_UNLIKELY (!itm || itm->typnum <= momty__itemlowtype))
    MOM_FATAL ("invalid item @%p to fill item data", itm);
  // fill the attributes
  ...
  switch (jtype)
	...
	if (jtypv.panyitem == (mom_anyitem_t *) mom_item__itemref)
	...
	else if (jtypv.panyitem == (mom_anyitem_t *) mom_item__closure)
	  {
	    ...
	    if (nbsons < TINY_MAX)
	      {
		momval_t sontab[TINY_MAX] = { MOM_NULLV };
...
rootspace_fetch_build (unsigned spanum, const char *uuidstr)
{
  ...
  if (stepres == SQLITE_ROW)
	...
	sqlite3_column_text (fetchbuild_loadstmt, 0);
      ...
      if (MOM_UNLIKELY (!res))
	MOM_FATAL ("failed to duplicate build of %s which is %s",
		   uuidstr, colbuild);
  ...
  if (stepres == SQLITE_DONE)
    ...
    MOM_FATAL ("failed to fetch build %s: %s (%d:%s)",
...
rootspace_fetch_fill (unsigned spanum, const char *uuidstr)
{
  ...
  assert (spanum == MOM_SPACE_ROOT);
  assert (mom_dbsqlite != NULL);
  if (MOM_UNLIKELY (fetchfill_loadstmt == NULL))
    {
      if (sqlite3_prepare_v2 (mom_dbsqlite,
			      "SELECT jfill FROM t_item WHERE uid = ?1",
			      -1, &fetchfill_loadstmt, NULL))
	MOM_FATAL ("failed to prepare fetchfill query: %s",
		   sqlite3_errmsg (mom_dbsqlite));
    };
  // uuidstr at index 1
  ...
  if (stepres == SQLITE_ROW)
	...
	sqlite3_column_text (fetchfill_loadstmt, 0);
      ...
      if (MOM_UNLIKELY (!res))
	MOM_FATAL ("failed to duplicate fill of %s which is %s",
		   uuidstr, colfill);
  ...
  if (stepres == SQLITE_DONE)
    ...
    MOM_FATAL ("failed to fetch fill %s: %s (%d:%s)",
	       uuidstr, sqlite3_errmsg (mom_dbsqlite),
	       stepres, sqlite3_errstr (stepres));
}
			    ...
			    const char *buildstr, const char *fillstr)
{
  ...
  if (MOM_UNLIKELY (buildfill_dumpstmt == NULL))
    {
      if (sqlite3_prepare_v2 (mom_dbsqlite,
			      "INSERT INTO t_item (uid, type, jbuild, jfill) VALUES (?1, ?2, ?3, ?4)",
			      -1, &buildfill_dumpstmt, NULL))
	MOM_FATAL ("failed to prepare build&fill insertion: %s",
		   sqlite3_errmsg (mom_dbsqlite));
    }
  // uidstr at index 1
  if (sqlite3_bind_text (buildfill_dumpstmt, 1, ustr, -1, SQLITE_STATIC))
    MOM_FATAL ("failed to bind uuid: %s", sqlite3_errmsg (mom_dbsqlite));
  // type at index 2
...
fetch_param (const char *parname)
{
  ...
  if (!parname || !parname[0])
    return NULL;
  if (MOM_UNLIKELY (fetchparam_loadstmt == NULL))
    {
      if (sqlite3_prepare_v2 (mom_dbsqlite,
			      "SELECT parvalue FROM t_param WHERE parname = ?1",
			      -1, &fetchparam_loadstmt, NULL))
	MOM_FATAL ("failed to prepare fetchparam query: %s",
		   sqlite3_errmsg (mom_dbsqlite));
    }
  // parname at index 1
  if (sqlite3_bind_text (fetchparam_loadstmt, 1, parname, -1, SQLITE_STATIC))
    MOM_FATAL ("failed to bind parrame: %s", sqlite3_errmsg (mom_dbsqlite));
  ...
  if (stepres == SQLITE_ROW)
	...
	sqlite3_column_text (fetchparam_loadstmt, 0);
      ...
      if (MOM_UNLIKELY (!res))
	MOM_FATAL ("failed to duplicate param of name %s which is %s",
		   parname, colparam);
  ...
  if (stepres == SQLITE_DONE)
    ...
    MOM_FATAL ("failed to fetch parameter %s: %s (%d:%s)",
...
param_printf (const char *paramname, const char *fmt, ...)
{
  ...
  va_end (args);
  if (MOM_UNLIKELY (!buf))
    MOM_FATAL ("failed to print param %s", fmt);
  if (MOM_UNLIKELY (putparam_dumpstmt == NULL))
    {
      if (sqlite3_prepare_v2 (mom_dbsqlite,
			      "INSERT INTO t_param (parname, parvalue) VALUES (?1, ?2)",
			      -1, &putparam_dumpstmt, NULL))
	MOM_FATAL ("failed to prepare putparam query: %s",
		   sqlite3_errmsg (mom_dbsqlite));
    }
  // parname at index 1
  if (sqlite3_bind_text (putparam_dumpstmt, 1, paramname, -1, SQLITE_STATIC))
    MOM_FATAL ("failed to bind parame: %s", sqlite3_errmsg (mom_dbsqlite));
  // buf at index 2
  if (sqlite3_bind_text (putparam_dumpstmt, 2, buf, -1, SQLITE_STATIC))
    MOM_FATAL ("failed to bind buf: %s", sqlite3_errmsg (mom_dbsqlite));
  // now insert the param
  int stepres = sqlite3_step (putparam_dumpstmt);
  if (stepres != SQLITE_DONE)
    MOM_FATAL ("failed to insert parameter %s: %s (%d:%s)",
	       paramname, sqlite3_errmsg (mom_dbsqlite),
	       stepres, sqlite3_errstr (stepres));
  if (sqlite3_reset (putparam_dumpstmt))
    MOM_FATAL ("failed to reset put parameter statement: %s",
	       sqlite3_errmsg (mom_dbsqlite));
  free (buf);
}
...
ldn_cb (void *data, int nbcol, char **colarrs, char **colnames)
{
  ...
  assert (spaname != NULL);
  for (unsigned spix = 1; spix < MOM_SPACE_MAX; spix++)
    {
      if (!mom_spacedescr_array[spix])
	MOM_FATAL ("unknown space %s for uuid %s name %s", spaname,
...
{
  mom_spacedescr_array[MOM_SPACE_ROOT] = &mom_root_space_descr;
  mom_spacename_array[MOM_SPACE_ROOT] =
    (momstring_t *) mom_make_string (MOM_ROOT_SPACE_NAME);
}
...
{
  assert (ld && ld->ldr_magic == LOADER_MAGIC);
#define MOM_NAMED(Nam,Typ,Uuid) do {				\
  if (mom_item__##Nam)							\
...
mom_initial_load (const char *state)
{
  ...
  if (errcod)
    MOM_FATAL ("failed to open sqlite3 %s:%s", state,
	       sqlite3_errmsg (mom_dbsqlite));
  char *errmsg = NULL;
  const char *vers = fetch_param (MOM_VERSION_PARAM);
  if (MOM_UNLIKELY (vers && strcmp (vers, MOM_DUMP_VERSION)))
    MOM_FATAL
      ("in state %s dump format version mismatch got %s expected %s", state,
       vers, MOM_DUMP_VERSION);
  if (sqlite3_exec
      (mom_dbsqlite, "SELECT COUNT(*) AS nb_items FROM t_item", setintptr_cb,
       &nbitems, &errmsg))
    MOM_FATAL ("counting items in %s failed with %s", state, errmsg);
  if (sqlite3_exec (mom_dbsqlite,
		    "SELECT COUNT(*) AS nb_names FROM t_name",
		    setintptr_cb, &nbnames, &errmsg))
    MOM_FATAL ("counting names in %s failed with %s", state, errmsg);
  if (sqlite3_exec (mom_dbsqlite,
		    "SELECT COUNT(*) AS nb_modules FROM t_module",
		    setintptr_cb, &nbmodules, &errmsg))
    MOM_FATAL ("counting modules in %s failed with %s", state, errmsg);
  MOM_INFORM ("state %s has %d items and %d names in %d modules", state,
	      (int) nbitems, (int) nbnames, (int) nbmodules);
  if (MOM_UNLIKELY (nbitems == 0 || nbnames == 0))
    MOM_FATAL ("no items or names to load from %s", state);
  load_modules ();
  ...
  if (sqlite3_exec (mom_dbsqlite,
		    ...
		    ldn_cb, &ldn, &errmsg))
    MOM_FATAL ("fetching names in %s failed with %s", state, errmsg);
  MOM_DEBUG (load, "ldn_count=%d", ldn.ldn_count);
  for (unsigned ix = 0; ix < ldn.ldn_count; ix++)
	...
	{
	  MOM_DEBUG (load, "loading item %s", curuuidstr);
	  curitm = mom_load_item (&ld, curuid, curspace);
	  MOM_DBG_ITEM (load, "loaded item", curitm);
	  if (!curitm)
	    MOM_FATAL ("failed to load named item %s of uid %s space %s",
  ...
  while (ld.ldr_qfirst != NULL)
    {
      ...
      assert (itmld->i_space > 0 && itmld->i_space < MOM_SPACE_MAX);
	...
	{
	  struct jsonparser_st jp = { 0 };
	  MOM_DEBUG (load, "fillstr %s", fillstr);
	  FILE *fm = fmemopen (fillstr, strlen (fillstr), "r");
	  if (MOM_UNLIKELY (!fm))
	    MOM_FATAL ("fmemopen failed for uid %s fill string %s",
		       curustr, fillstr);
	  mom_initialize_json_parser (&jp, fm, NULL);
	  char *errmsg = NULL;
	  momval_t jval = mom_parse_json (&jp, &errmsg);
  ...
  mom_dbsqlite = NULL;
  MOM_DEBUG (load, "end loading %s", state);
}
	       ...
	       void *data)
{
  ...
  if (!itm || itm->typnum <= momty__itemlowtype || itm->i_space == 0
      || itm->i_space >= MOM_SPACE_MAX || !mom_spacename_array[itm->i_space])
    return;
  ...
  if (sqlite3_bind_text
    ...
    MOM_FATAL ("failed to bind name: %s", sqlite3_errmsg (mom_dbsqlite));
  // uid string at index 2
  char ustr[UUID_PARSED_LEN];
  memset (ustr, 0, sizeof (ustr));
  uuid_unparse (itm->i_uuid, ustr);
  if (sqlite3_bind_text (stmt, 2, ustr, -1, SQLITE_STATIC))
    MOM_FATAL ("failed to bind uid: %s", sqlite3_errmsg (mom_dbsqlite));
  // spacename at index 3
  if (sqlite3_bind_text
      (stmt, 3,
       mom_string_cstr ((momval_t) mom_spacename_array[itm->i_space]), -1,
       SQLITE_STATIC))
    MOM_FATAL ("failed to bind space name: %s",
	       sqlite3_errmsg (mom_dbsqlite));
  int stepres = sqlite3_step (stmt);
  if (stepres != SQLITE_DONE)
    MOM_FATAL ("failed to insert global name %s: %s (%d:%s)",
	       mom_string_cstr ((momval_t) name),
	       sqlite3_errmsg (mom_dbsqlite),
	       stepres, sqlite3_errstr (stepres));
  if (sqlite3_reset (stmt))
    MOM_FATAL ("failed to reset statement: %s",
	       sqlite3_errmsg (mom_dbsqlite));
}
		  ...
		  const char *state)
{
  ...
  MOM_INFORM ("start of full state dump to %s from %s:%d reason %s",
  ...
  if (sqlite3_exec
    ...
    MOM_FATAL ("failed to create t_item: %s", errmsg);
  if (sqlite3_exec
      (mom_dbsqlite,
    ...
    MOM_FATAL ("failed to create t_name: %s", errmsg);
  if (sqlite3_exec
      (mom_dbsqlite,
  ...
  if (sqlite3_prepare_v2 (mom_dbsqlite,
			  ...
			  -1, &stmt, NULL))
    MOM_FATAL ("failed to prepare name insertion: %s",
	       sqlite3_errmsg (mom_dbsqlite));
  {
  ...
  MOM_DEBUG (dump, "scanning loop start");
  ...
  while (dmp.dmp_qfirst != NULL)
	...
	mom_typedescr_array[curitm->typnum];
      ...
      assert (curitm->i_space > 0 && curitm->i_space < MOM_SPACE_MAX);
  ...
  MOM_DEBUG (dump, "after scanning loop nbdumpeditems=%d",
	     (int) nbdumpeditems);
  ...
  for (unsigned dix = 0; dix < dmp.dmp_size; dix++)
	...
	if (!foutbuild)
	  MOM_FATAL ("failed to open stream for JSON build");
	...
	MOM_DEBUG (dump, "strbuild=%s", strbuild);
      }
      memset (&outj, 0, sizeof (outj));
      {
	char *buffill = NULL;
	...
	MOM_DEBUG (dump, "strfill=%s", strbuild);
  ...
  MOM_INFORM ("end of full state dump to %s from %s:%d reason %s", state,
...
traverse_module_tree (gpointer key, gpointer val, gpointer data)
{
  ...
  if (sqlite3_bind_text (modstmt, 1, modname, -1, SQLITE_STATIC))
    MOM_FATAL ("failed to bind modname: %s", sqlite3_errmsg (mom_dbsqlite));
  int stepres = sqlite3_step (modstmt);
  if (stepres != SQLITE_DONE)
    MOM_FATAL ("failed to insert module name %s: %s (%d:%s)",
	       modname,
	       sqlite3_errmsg (mom_dbsqlite),
	       stepres, sqlite3_errstr (stepres));
...
dump_modules (void)
{
  ...
  if (sqlite3_prepare_v2 (mom_dbsqlite,
			  ...
			  -1, &modstmt, NULL))
    MOM_FATAL ("failed to prepare module insertion query: %s",
	       sqlite3_errmsg (mom_dbsqlite));
...
load_modules (void)
{
  ...
  if (sqlite3_exec
      (mom_dbsqlite, "SELECT modname FROM t_module", loadmodule_cb,
       NULL, &errmsg))
    MOM_FATAL ("loading modules failed with %s", errmsg);
}
